...
public class MyErrorStrategy : DefaultErrorStrategy
{
    ...
    {
        string msg = "Entrada no válida " + GetTokenErrorDisplay(e.OffendingToken) + " se esperaba " + e.GetExpectedTokens().ToString(recognizer.Vocabulary);
    ...
    protected override void ReportMissingToken(Parser recognizer)
    {
        ...
        string msg = "falta " + GetExpectedTokens(recognizer).ToString(recognizer.Vocabulary)+ " en " + GetTokenErrorDisplay(currentToken);
    ...
    {
        IToken currentToken = recognizer.CurrentToken;
        string msg = "Entrada no válida  " + GetTokenErrorDisplay(currentToken) + " se esperaba " + GetExpectedTokens(recognizer).ToString(recognizer.Vocabulary);
        recognizer.NotifyErrorListeners(currentToken, msg, null);
    }
    ...
    protected override void ReportNoViableAlternative(Parser recognizer, NoViableAltException e)
    {
        ...
        string msg = "Alternativa no viable en la entrada" + EscapeWSAndQuote(inputStream == null ? "<unknown input>" : (e.StartToken.Type != -1 ? inputStream.GetText(e.StartToken, e.OffendingToken) : "<EOF>"));
